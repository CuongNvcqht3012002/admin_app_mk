...
import BookingByAdminEditForm from './BookingByAdminEditForm';
...
export default function BookingByAdminMenu(props) {
  const { bookingByAdmin, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_SERVICE_BASE_URL}/booking/${bookingByAdmin.id}`, {
        headers: {
  ...
  const handleDetail = () => {
    ...
    setIsDetail(true);
    axios
      .get(`${process.env.REACT_APP_SERVICE_BASE_URL}/booking-quote/admin/${bookingByAdmin.id}`, {
        headers: {
          Authorization: `Bearer ${token.accessToken}`,
        },
      })
    ...
    <>
      ...
      >
        <Box sx={style} style={{ width: '70%' }}>
          <BookingByAdminEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Modal
        open={detail}
      ...
      >
         <MenuItem sx={{ color: 'text.secondary' }} onClick={handleDetail}>
          ...
          <ListItemText primary="Danh sách báo giá" primaryTypographyProps={{ variant: 'body2' }} />
        </MenuItem>
