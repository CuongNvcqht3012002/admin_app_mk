...
export default function SEOMenu(props) {
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_SERVICE_BASE_URL}/region/${region.id}`, {
        headers: {
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa khu vực này?</DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style} style={{ width: '70%' }}>
          <RegionEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Modal
