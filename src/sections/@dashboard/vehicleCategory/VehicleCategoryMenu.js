...
import VehicleCategoryEditForm from './VehicleCategoryEditForm';
...
export default function SEOMenu(props) {
  const { vehicleCategory, status, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_SERVICE_BASE_URL}/vehicle-category/${vehicleCategory.id}`, {
        headers: {
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa danh mục này?</DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style} style={{ width: '70%' }}>
          <VehicleCategoryEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Modal
