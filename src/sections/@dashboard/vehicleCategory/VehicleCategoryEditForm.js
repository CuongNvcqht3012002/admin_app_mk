...
VehicleCategoryEditForm.propTypes = {
  vehicleCategory: propTypes.object,
  closeModal: propTypes.func,
  reloadData: propTypes.func,
};

export default function VehicleCategoryEditForm(props) {
  const { vehicleCategory, closeModal, reloadData } = props;
  const [isUploadingImage, setIsUploadingImage] = useState(false);
  const validation = Yup.object().shape({
    seat: Yup.string().required('Vui lòng nhập đầy đủ thông tin trường'),
  });
  const formik = useFormik({
    ...
    onSubmit: (values, formikHelpers) => {
      (vehicleCategory != null
        ? axios.patch(`${process.env.REACT_APP_SERVICE_BASE_URL}/vehicle-category/${vehicleCategory.id}`, values, {
            headers: {
              Authorization: `Bearer ${token.accessToken}`,
            },
          })
        : axios.post(`${process.env.REACT_APP_SERVICE_BASE_URL}/vehicle-category`, values, {
            headers: {
              Authorization: `Bearer ${token.accessToken}`,
            },
          })
