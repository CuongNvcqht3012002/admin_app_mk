...
PremiumEditForm.propTypes = {
  premiumId: propTypes.array,
  premium: propTypes.object,
  closeModal: propTypes.func,
  reloadData: propTypes.func,
};

export default function PremiumEditForm(props) {
  const { premium, closeModal, reloadData } = props;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, formikHelpers) => {
      (premium != null
        ? axios.patch(`${process.env.REACT_APP_SERVICE_BASE_URL}/premium-package/${premium.id}`, values, {
            headers: {
              Authorization: `Bearer ${token.accessToken}`,
            },
          })
        : axios.post(`${process.env.REACT_APP_SERVICE_BASE_URL}/premium-package`, values, {
            headers: {
              Authorization: `Bearer ${token.accessToken}`,
            },
          })
