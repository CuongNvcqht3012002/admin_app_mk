...
export default function SEOMenu(props) {
  const { enterprise, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_SERVICE_BASE_URL}/enterprise/${enterprise.id}`, {
        headers: {
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa nhà xe này?</DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style} style={{ width: '70%' }}>
          <EnterpriseEditForm  {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Modal
