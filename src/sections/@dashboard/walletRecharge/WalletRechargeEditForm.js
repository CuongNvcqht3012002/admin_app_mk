...
WalletRechargeEditForm.propTypes = {
  walletRechargeId: propTypes.array,
  walletRecharge: propTypes.object,
  closeModal: propTypes.func,
  reloadData: propTypes.func,
};

export default function WalletRechargeEditForm(props) {
  const { walletRecharge, closeModal, reloadData } = props;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, formikHelpers) => {
      (walletRecharge != null
        ? axios.put(`${process.env.REACT_APP_SERVICE_BASE_URL}/walletRecharge/${walletRecharge.id}`, values, {
            headers: {
          Authorization: `Bearer ${token.accessToken}`,
        },
          })
        : axios.post(`${process.env.REACT_APP_SERVICE_BASE_URL}/walletRecharge/withdraw-request`, values, {
            headers: {
          Authorization: `Bearer ${token.accessToken}`,
        },
          })
