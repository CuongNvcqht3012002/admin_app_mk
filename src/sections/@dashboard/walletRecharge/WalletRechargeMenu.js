...
import WalletRechargeEditForm from './WalletRechargeEditForm';
...
export default function WalletRechargeMenu(props) {
  const { walletRecharge, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_SERVICE_BASE_URL}/post/${walletRecharge.id}`, {
        headers: {
    ...
    <>
      ...
      >
        <Box sx={style} style={{ width: '70%' }}>
          <WalletRechargeEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}
